...
async function startServer() {
  ...
  // Config route for frontend (provides API keys safely)
  app.get("/api/config", (req, res) => {
    console.log("Config requested. API_KEY:", process.env.API_KEY ? "exists" : "missing", "GEMINI_API_KEY:", process.env.GEMINI_API_KEY ? "exists" : "missing");
    
    // Prefer API_KEY if GEMINI_API_KEY is a placeholder or missing
  ...
  app.post("/api/notify-payment", async (req, res) => {
    const { email, firstName, lastName, sessionId, userId } = req.body;
    const smtpUser = process.env.SMTP_USER?.trim();
    const smtpFrom = process.env.SMTP_FROM?.trim();
    ...
    // Update Firestore with Stripe subscription details
    const stripe = getStripe();
    if (stripe && sessionId && userId && db) {
      try {
        const session = await stripe.checkout.sessions.retrieve(sessionId);
        if (session.subscription) {
          ...
          console.log(`✅ Updated Firestore with Stripe subscription for user ${userId}`);
        }
      } catch (err) {
        console.error("❌ Failed to update Stripe subscription in Firestore:", err);
      }
    }
  ...
  app.post("/api/cancel-subscription", async (req, res) => {
    ...
    try {
      if (subscriptionId) {
        ...
        // Fallback: Find customer by email and cancel their active subscriptions
